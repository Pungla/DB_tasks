...
Каждый запрос сверялся с ожидаемым выводом из условия (`task.md`) на тех же
тестовых данных. Результаты совпадают.

Замечание по базе `04_organization`: в задачах 1-3 список задач сотрудника
сортируется в бинарном порядке (`COLLATE "C"`). Это нужно, чтобы строка
"Задача 14: ..." шла раньше "Задача 1: ..." — так же, как в ожидаемом выводе.
При сортировке по локали порядок этих строк получается другим.
